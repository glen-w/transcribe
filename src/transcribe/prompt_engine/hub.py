...
def resolve_prompt(
    ...
    *,
    ...
    Project and workspace overrides always win for ``prompt_id`` (their bumped
    ``version`` participates in detection cache identity). The ``version`` pin
    applies only when selecting among custom/builtin/OCR/cleanup definitions.
    """
    rt = runtime or build_runtime_paths()
