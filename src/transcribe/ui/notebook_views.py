...
from transcribe.ui.notebook_view_page import render_notebook_view_page
...
def render_view_mood(runtime, paths, projects, project, *, get_analysis_coordinator) -> None:
    ...
    render_notebook_view_page(
        ...
        empty_kind=None if published else "no_results_yet",
        empty_title="No published analysis yet",
        empty_body="Run Analyse on this notebook to see mood and tone.",
    )


def render_view_moments(runtime, paths, projects, project, *, get_analysis_coordinator) -> None:
    _ = runtime
    spec = page_spec_for("Moments")
    assert spec is not None
    published = notebook_has_published_analysis(paths.root)
    ctx = load_view_health(
        paths, projects, project, get_analysis_coordinator=get_analysis_coordinator
    ...
    render_notebook_view_page(
        ...
        health=ctx["batch_health"],
        show_analyse_cta=not published,
        analyse_cta_key="moments_analyse_cta",
        body=_body if published else None,
        empty_kind=None if published else "no_results_yet",
        empty_title="No published analysis yet",
        empty_body="Run Analyse on this notebook to see moments.",
    )


def render_view_people(runtime, paths, projects, project, *, get_analysis_coordinator) -> None:
    spec = page_spec_for("People")
    assert spec is not None
    published = notebook_has_published_analysis(paths.root)
    ctx = load_view_health(
        paths, projects, project, get_analysis_coordinator=get_analysis_coordinator
    ...
    render_notebook_view_page(
        ...
        health=ctx["batch_health"],
        show_analyse_cta=not published,
        analyse_cta_key="people_analyse_cta",
        body=_body if published else None,
        empty_kind=None if published else "no_results_yet",
        empty_title="No published NER yet",
        empty_body="Run Analyse (including NER) to map people and places in this notebook.",
    )
...
def render_view_summaries(runtime, paths, projects, project, *, get_analysis_coordinator) -> None:
    ...
    render_notebook_view_page(
        ...
        health=ctx["batch_health"],
        show_analyse_cta=not published,
        analyse_cta_key="summaries_analyse_cta",
        body=_body if published else None,
        empty_kind=None if published else "no_results_yet",
        empty_title="No published analysis yet",
        empty_body="Run Analyse on this notebook to see summaries.",
    )
