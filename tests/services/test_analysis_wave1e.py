...
def test_chunking_policy_packs_units(tmp_path: Path):
    ...
    chunks = pack_units_v1(doc, max_chars=80)
    assert chunks
...
def test_deterministic_synthesis_offline(tmp_path: Path):
    ...
    try:
        ...
        assert batch["narrative_summary"]["outcome"] == "success"
        assert batch["narrative_summary"]["payload"]["honesty_label"] == (
            "deterministic_from_summary"
        )
...
def test_llm_custom_qa_grounding_and_abstain(tmp_path: Path):
    ...
    try:
        ...
        # Register override by running through module directly then via runner identity
        result = mod.run(doc)
    ...
    try:
        result = LLMCustomQAModule(question_text="Anything?").run(doc)
