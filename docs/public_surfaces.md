...
**View** (after the notebook picker): Reading · Overview · Themes · Mood · Moments · People · Summaries · Ask · Detect. Nav labels are short; page titles stay long (Mood → “Mood & tone”, People → “People & places”, Ask → “Ask notebook”). View pages consume **current** `published.json`. There is no analysis-run picker. Missing published analysis **disables** those nav buttons (`help`: “Select a notebook” / “Analyse this notebook first”) but **does not bounce** the current page — empty state + Analyse CTA. Overview requires a notebook only (`page_metrics` is independent of text Analyse).
