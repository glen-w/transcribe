...
3. Run analysis, then inspect published results in Overview / Themes / Mood & tone / Moments / People & places / Summaries / Ask notebook. A shared status strip shows whether results are current. Each tab shows module-appropriate charts and lists (not raw JSON). Overview and Mood include **Compare with corpus / period** for numeric metrics vs other notebooks. People & places adds **entity tone** when that module has run. Technical module details live under **Advanced**. Use **Notebooks → Places** for a map of places mentioned across all notebooks (opt-in OpenStreetMap geocoding; results cached locally).
