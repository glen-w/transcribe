...
**Review** is a work queue for the open notebook: **scan → independent OCR evidence → LLM merged draft → human-reviewed transcription**. Filter to unreviewed, needs attention, high disagreement, date approval, no text, failed OCR, reviewed, or skipped. Approve or ignore all suggested dates in one pass (suspicious date regressions ask for a second confirm). Unapproved suggested dates still appear in the Library timeline; time-of-day stamps are ignored.

The Review page is a two-pane workbench: large scan on the left; a **lane switcher** on the right for **Transcription**, **Date**, **Tags**, **OCR**, **Cleanup**, and **Other** (only one lane body runs; changing pages keeps the selected lane). **Thumbnails** opens a page grid (zoom in/out for fewer/more thumbs); click the page button under a thumb to open that page. The **Transcription** lane holds the editor, OCR evidence strip, and disagreement navigation. When several vision models have read the page, only **source disagreements** are listed (wrapping/whitespace and punctuation-only / prompt-leak noise are not navigable steps). The merged draft may recommend a variant or be flagged when it **departs** from every raw reading. **Save** keeps you on the page; **Save + Mark reviewed** stores the current transcription and OCR-evidence fingerprints, then advances. Keyboard: ←/→ pages, J/K disagreements, 1/2/3 choose a variant, R mark reviewed, Cmd/Ctrl+S save (other shortcuts are ignored while typing). The **Date** lane covers manual entry, approve/ignore suggestions, and **💾 Save date**; the **Tags** lane has tag pills and **💾 Save tags**. **OCR** holds per-notebook **OCR settings** (**When setting a notebook default**, **Seed transcription from merged draft after multipass**), **Re-run OCR** (pick a vision model, then this page / all pages / pages not marked reviewed), and **Rank and merge** / **Build merged draft**. **Cleanup** re-applies visual declutter (scanner beds / white gutters / corner wedges) on this page or all pages in the notebook without re-running OCR. **Other** holds notebook cover and **Delete page** — see [runtime/ocr.md](runtime/ocr.md#notebook-ocr-settings). On Reading/Library pages with multiple OCR attempts, use **Compare in Review**. In Reading/Library, click a coloured tag pill to AND-filter the served page set; **Clear tag filter** restores the full set ([tag-catalog](contracts/tag-catalog.md)).
...
**Library** defaults to a cover-thumbnail overview. Click an activity bar to filter notebooks/pages to that date bin. Switch to **Activity** for the per-notebook histogram list. The cover grid loads `ui.archive_notebooks_initial` cards first (default **all**; change under **Settings → Configuration → Library**), then **Show more** / **Show fewer**.

**Search** finds text across notebooks. Use Period / Year / Range (same idea as Library), tags, and media filters. Open a hit to browse matching pages with Prev/Next, or switch to **This notebook** to step through all pages in chronological order.
...
4. Open **View → Detect** to scan for poetry, to-do lists, other lists, quotations, beer labels, first-person `I` (per-page count from OCR text, no LLM), swear words, people names (or custom detectors). Review findings, jump to source pages, and **Accept** or **Reject** (count detectors show a per-page table instead of review cards). Multi-page findings have **Accept this page** / **Reject this page** on each page tab so you can drop a misidentified page without rejecting the rest; **Accept remaining** then keeps the others. Accept applies remaining page tags. Detectors can auto-apply page tags when configured ([runtime/analysis.md](runtime/analysis.md); catalogue under **Settings → Tags** · [tag-catalog](contracts/tag-catalog.md)). Names / people tags each detected person name rather than a generic `names` tag.

Edit what each preset includes under **Settings → Analysis**. Manage prompts under **Settings → Prompts** and custom detectors under **Settings → Detection**. **Settings** tabs: Configuration · Analysis · Detection · Tags · Prompts · Interface · Models · Profiles · Export. Configuration holds **Backup**, import/declutter, Library cover-grid paging, and Overview cards. Models holds the workspace Ollama URL, OCR preprocess seed (`none` / `gentle_contrast`), LLM budgets, and Apply-OCR to the open notebook. Profiles activates named overlays (`workflow` / `ocr` / `llm` / `export`). **Interface** customises action menus (which actions appear, and whether strips show icons, text, or both — Library cover cards default to icons only). Export shows read-only typography defaults (`readable` / `compact` / `large_print`); change them on **Workflow → Export** or by activating an export profile.
