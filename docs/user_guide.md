...
3. Run analysis, then inspect published results in Overview / Themes / Mood & tone / Moments / People & places / Summaries / Ask notebook. A shared status strip shows whether results are current. Overview and Mood include charts; open **Compare with corpus / period** to bar-compare this notebook with the average of other notebooks (entire corpus, a year, or a date range). Technical module details live under **Advanced**. Use **Notebooks → Places** for a map of places mentioned across all notebooks (opt-in OpenStreetMap geocoding; results cached locally).
