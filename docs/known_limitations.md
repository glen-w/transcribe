...
- **Word themes** prefer a real word-cloud image (optional ``wordcloud`` package from ``.[ui]``) built from published token frequencies, same idea as TranscriptX. Without that package, Overview falls back to token-weight bars. Analysis still stores frequencies only — images are not durable artifacts.
